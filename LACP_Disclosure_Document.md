...
for each pair (Agent_A, Agent_B):
    ...
    if similarity > SYCOPHANCY_THRESHOLD:
...
Bounds are enforced within a narrow sub-range of [0, 1] to prevent degenerate behaviour (e.g., temperature=0 or threshold=1.0).

### 6.2 OCEAN → Derived Behaviour Mapping

The `deriveFromOcean(role, ocean)` function computes operational parameters from OCEAN scores using weighted linear combinations with role-specific base values. Each derived parameter is a deterministic function of one or more OCEAN dimensions:

*   **Temperature**: Primarily driven by Openness (positive) and Conscientiousness (negative)
*   **Top-P**: Primarily driven by Openness
*   **Confidence Threshold**: Primarily driven by Conscientiousness and Neuroticism
*   **Tribunal Vote Weight**: Driven by Conscientiousness (positive) and Agreeableness (negative)
*   **Escalation Threshold**: Derived from Neuroticism and Conscientiousness
*   **Detail Level**: Discrete {concise, standard, verbose} from Extraversion + Conscientiousness
*   **Risk Tolerance**: Discrete {conservative, balanced, aggressive} from Openness, Neuroticism, Conscientiousness
*   **Self-Review Passes**: Integer [0, 2] from Neuroticism + Conscientiousness

Default OCEAN scores are calibrated so that derived parameters approximate existing hardcoded values, enabling zero-behaviour-change deployment. Implementations define their own weighted coefficients.

### 6.3 Agent Archetypes

Agents are initialised with **archetypes** — named personality templates that encode domain-appropriate OCEAN scores. Archetypes are domain-specific: different swarms (e.g., code review, content moderation, financial analysis) define their own archetype sets with appropriate OCEAN calibrations.

Archetype categories cluster into four temperature bands:

| Category | Typical Use | Temperature Band |
|----------|------------|-----------------|
| Precision | Auditing, safety, compliance | Conservative (0.15–0.25) |
| Coordination | Planning, orchestration | Moderate (0.25–0.45) |
| Analytical | Research, documentation | Balanced (0.45–0.55) |
| Creative | Design, exploration, strategy | Creative (0.55–0.70) |

### 6.4 Personality Evolution

Agent personalities evolve conservatively based on feedback signals:

| Trigger | Typical OCEAN Effect |
|---------|---------------------|
| Human correction (major) | +C, +N, -O |
| Tribunal vote won | +E |
| Tribunal vote lost | +A, +N |
| Task completion (high confidence) | +O |
| Task completion (failure) | +C |
| Accuracy feedback (inaccurate) | +C, -O |

**Safeguards:**
*   Minimum task count before evolution begins (cold-start protection)
*   Maximum shift per event is bounded (implementation-defined)
*   OCEAN bounds enforced after each evolution step (implementation-defined)
*   Version snapshots at regular intervals for rollback capability
...
LACP v2 extends the Tribunal mechanism (§5) with **personality-weighted voting**, where each agent's vote is multiplied by its derived `tribunalVoteWeight`.
...
## 11. Domain Application Example

This section outlines how LACP can be applied to domain-specific multi-agent pipelines.

### 11.1 Pipeline Pattern

LACP supports directed acyclic graph (DAG) pipelines where specialised agents execute in stages with selective parallelism. A typical domain pipeline consists of:

1. **Discovery stage** — An EXPLORER-role agent analyses input data
2. **Analysis stages** (parallel) — Multiple BUILDER/VALIDATOR agents process independently
3. **Synthesis stage** — A downstream agent combines parallel outputs
4. **Quality gate** — A REVIEWER-role agent cross-validates all upstream outputs

VALIDATOR-role agents may use higher-accuracy model tiers, reflecting the LACP principle that verification agents warrant greater computational investment.

### 11.2 Agent Capability Dependencies

Each agent declares its input requirements and output provisions, enabling the Coordinator to calculate the execution DAG at runtime. Independent stages execute in parallel to reduce latency.

### 11.3 Domain-Specific Tribunal Triggers

The Tribunal mechanism (§5) can be extended with domain-specific trigger types. Common patterns include:

| Trigger Pattern | Condition | Escalation |
|----------------|-----------|------------|
| Low confidence | Agent confidence below threshold | Panel review |
| Inter-agent conflict | Two agents produce contradictory assessments | Panel + human escalation if no consensus |
| High-stakes decision | Value or risk exceeds threshold | Full panel + human review |
| Rare/unknown input | Low match against known patterns | Specialist panel + human review |

Tribunal thresholds are modulated by AAPS personality parameters (§6): agents with high Neuroticism convene tribunals more readily, while agents with low Neuroticism rarely trigger escalation.

### 11.4 Memory Integration

Swarms register with the orchestration layer, declaring capabilities. At execution time:

1. **Memory Retrieval**: Relevant memories and patterns are injected into each agent's context.
2. **Learning Emission**: After pipeline completion, high-confidence results are stored as typed memories.
3. **Cross-Swarm Transfer**: Via the Gene Pool (§8), patterns are shared across swarm instances.

### 11.5 Human Review Escalation

Pipelines compute an overall confidence score. Human review is triggered when:
*   The quality gate agent flags issues
*   Overall pipeline confidence falls below a configured threshold
